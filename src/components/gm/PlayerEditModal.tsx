    ...
    <ModalWrapper title={`${player.nickname}`} onClose={onClose}>
      <div className="space-y-4 max-h-[70vh] overflow-y-auto">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-1 p-2 bg-neutral-900 rounded-lg border border-neutral-700 max-h-40 overflow-y-auto">
            {roles.map((role) => (
              <button
                key={role}
                onClick={() => gmUpdatePlayerRole(player.id, role)}
                className={`p-2 rounded text-xs font-bold transition-all text-left truncate ${
                  player.role === role
                    ? "bg-teal-600 border border-teal-500 text-white shadow-[0_0_10px_rgba(20,184,166,0.3)]"
                    : "bg-teal-950/20 border border-teal-900/50 text-teal-400 hover:border-teal-500 hover:bg-teal-950/30"
                }`}
              >
                {role}
              </button>
